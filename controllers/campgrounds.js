...
module.exports.index = async (req, res) => {
    try {
        ...
        const campgrounds = await Campground.find({}).populate('author').populate({
            path: 'reviews',
            populate: {
                path: 'author'
            }
        ...
        const updatedCampgrounds = campgrounds.map(campground => {
            ...
            const averageRating = ratings.length ? (ratings.reduce((sum, rating) => sum + rating, 0) / ratings.length).toFixed(1) : 0;
            return { ...campground.toObject(), averageRating };
        });

        res.render('campgrounds/index', { campgrounds: updatedCampgrounds, formatTimeDifference });
    } catch (err) {
        console.error(err);
...
module.exports.showCampground = async (req, res) => {
    ...
    const allUnavailableDates = [
        ...
        ...campground.manualUnavailableDates.map(date => {
    ...
    if (startDate && endDate) {
        isAvailable = !allUnavailableDates.some(dateRange =>
...
module.exports.updateCampground = async (req, res)=>{
    ...
    if (req.body.campground.manualUnavailableDates) {
        campground.manualUnavailableDates = req.body.campground.manualUnavailableDates.map(date => new Date(date));
    }
